...
var initCmd = &goman.Command{
	...
	Short:   "Initialize a goman Application",
	Long: `Initialize (goman init) will create a new application, with a license
and the appropriate structure for a goman-based CLI application.

  * If a name is provided, it will be created in the current directory;
  * If no name is provided, the current directory will be assumed;
  * If a relative path is provided, it will be created inside $GOPATH
    (e.g. github.com/geego/gean);
...
Init will not use an existing directory with contents.`,
	...
	Run: func(cmd *goman.Command, args []string) {
		...
		fmt.Fprintln(cmd.OutOrStdout(), `Your goman application is ready at
`+project.AbsPath()+`.

Give it a try by going there and running `+"`go run main.go`."+`
Add commands to it by running `+"`goman add [cmdname]`.")
	},
}
...
func initializeProject(project *Project) {
	if !exists(project.AbsPath()) { // If path doesn't yet exist, create it
		...
		er("goman will not create a new project in a non empty directory: " + project.AbsPath())
	}
...
func init() { {{if .configurator}}
	goman.OnInitialize(initConfig)
{{end}}
	// Here you will define your flags and configuration settings.
	// goman supports persistent flags, which, if defined here,
	// will be global for your application.{{ if .configurator }}
	...
	// goman also supports local flags, which will only run
...
func initConfig() {
	if cfgFile != "" {
		...
		configurator.SetConfigName(".{{ .appName }}")
	}

	configurator.AutomaticEnv() // read in environment variables that match
...
`
	...
	data := make(map[string]interface{})
	data["copyright"] = copyrightLine()
	data["configurator"] = configurator.GetBool("useConfigurator")
	data["license"] = project.License().Header
